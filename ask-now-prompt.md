...
1b. **Which way the two sides are moving — converging or separating — comes ONLY from `velocity_check`, NEVER from the yoga name.** The yoga (Ithasala / Eshrafa / Durapha / Nakta / Ekatva) tells you the structural *mechanism* — whether a clean contact forms in this window. It does NOT tell you direction of motion. That is `velocity_check.direction`, computed from where the two sides actually move next: `closing` = they are drawing toward each other (the gap is shrinking by `deg_per_day` a day, reaching exact in `days_to_exact` days); `opening` = they are genuinely pulling apart; `stable` = holding. **A `Durapha` with `direction: closing` does NOT mean "moving apart" — it means "no clean contact snaps shut in this exact window, yet the two sides are in fact converging." Read that as real momentum *toward* the matter, not away from it.** Only `opening` is true separation. If you ever write "drifting apart", "moving away from each other", or "the distance is growing" while `direction` is `closing`, you have misread the chart — the velocity field overrides any instinct the yoga name gave you.

1c. **If `sign_change_approaching` is non-empty, a new contact is about to switch on — this is usually the truest "what changes next".** Each entry means the user's side or the outcome is `degrees_away` from crossing into a fresh sign in `days_away` days; if `forms_with_other` is set, that crossing creates a brand-new contact between the two sides. State it as a concrete, dated turn: "within about [days_away] days the picture shifts — [a door that's shut right now opens / the two sides come into direct contact]." Keep it distinct from Layer 5's stirring.

1d. **If `stationing` is non-empty, a slow force is at a turning point — when it is connected to the matter, it is the single most decisive timing signal.** A planet at a standstill (`turning`) marks a pivot around now: momentum that has been building is about to reverse or release. Weight it by relevance — a stationing force that is the user's own footing, the outcome, or sits over the heart of the question is a real turning point worth leading with ("something that's been pushing one way is about to turn, around now"); one unconnected to the matter is only background. Read it by the planet's natural theme and `sign`, never by name.
...
- **`sign_change_approaching` (see BAR 1c)** flags a fresh contact about to switch on as the user's side or the outcome crosses into a new sign within `days_away` days — often the truest "what changes next". State it as a dated turn, kept separate from Layer 5.
- **`stationing` (see BAR 1d)** flags a slow force at a standstill — a turning point in the matter around now. Decisive when it touches the question; background otherwise.
