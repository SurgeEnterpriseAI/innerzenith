...
3. **Resolve contradictions explicitly.** If one dimension says no (e.g. the outcome where you are has peaked) and another says yes (e.g. something new is forming outside it), name BOTH and tell the user which one the chart actually answers. Don't average them into mush. **The most common and most powerful case: the main question's own outcome reads as a "no" (a fading or absent connection), but `notable_signals.opportunity_forming` is present — a benefic, often exalted, sitting right with the querent. When you see that combination, the true answer is almost always: "not here / not this — but something better is already forming right alongside you." Lead with that resolution.** Treat `opportunity_forming` as the headline whenever it's present.
