...
1d. **If `station_approaching` is non-empty, surface it as the PRIMARY timing signal — nothing outranks it.** A force central to the question (`role` = the user's own footing, the outcome, or the moment's ruling period) is within `days_to_station` days of a standstill and `stationing` either retrograde (a pause, then a reversal) or direct (a long stall now about to lift). This is the loudest thing about *when*: "something that's been pushing one way is about to turn — roughly [days_to_station] days out from the moment you asked." Lead your timing with it; read it by the `role` and the direction, never by planet name.
...
- **`station_approaching` (see BAR 1d)** flags the significator, outcome, or ruling period within `days_to_station` days of a standstill — the PRIMARY timing signal whenever present. Lead the timing with it.
